...
/**
 * This is the KeyNavigator class. It accumulates AWT key events (key press and
 * key release) and computes a new transform based on the accumulated events and
 * elapsed time.
 */
public class Simulation_Navigator {
	 ...
	 */
	public Simulation_Navigator(TransformGroup targetTG) {
	...
	private void accKeyAdd(Vector3d a, Vector3d da, Vector3d drag,
			double scaleVel) {
	 ...
	 * transform, accumulated keyboard inputs, and elapsed time. This new
	 * transform is written into the target transform group. This method should
	...
	public void integrateTransformChanges() {
		...
		else if ((key_state & UP_ARROW) == 0 && (key_state & DOWN_ARROW) != 0)
			accKeyAdd(a, bwdAcc, bwdDrag, scaleVel);

		if (((modifier_key_state & ALT) != 0) && (key_state & LEFT_ARROW) != 0
				&& (key_state & RIGHT_ARROW) == 0) {
			accKeyAdd(a, leftAcc, leftDrag, scaleVel);
		} else if (((modifier_key_state & ALT) != 0)
				&& (key_state & LEFT_ARROW) == 0
				&& (key_state & RIGHT_ARROW) != 0)
			accKeyAdd(a, rightAcc, rightDrag, scaleVel);

		if (((modifier_key_state & ALT) != 0) && (key_state & PAGE_UP) != 0
				&& (key_state & PAGE_DOWN) == 0)
		...
		/* Speed limits */
		if (navVec.z < scaleVel * fwdVMax)
			navVec.z = scaleVel * fwdVMax;
		if (navVec.z > scaleVel * bwdVMax)
			navVec.z = scaleVel * bwdVMax;
		if (navVec.x < scaleVel * leftVMax)
			navVec.x = scaleVel * leftVMax;
		if (navVec.x > scaleVel * rightVMax)
	...
	/**
	 * Processed a keyboard event. This routine should be called every time a
	 * KEY_PRESSED or KEY_RELEASED event is received.
	 * 
	 * @param keyEvent
	 *            the AWT key event
	 */
