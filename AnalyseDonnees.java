...
public class AnalyseDonnees {
	...
	public static Vect3 vitesseExp(int i, int j, Amas[][] t) {
		double expX, expY, expZ, dist, x, y, z, a;
		...
		expX = (t[i][j].getPos().getX() * a) * Math.abs((VAR * t[i][j].getPos().getX()));
		expY = (t[i][j].getPos().getY() * a) * Math.abs((VAR * t[i][j].getPos().getY()));
		expZ = (t[i][j].getPos().getZ() * a) * Math.abs((VAR * t[i][j].getPos().getZ()));
