                   ...
                   Runnable, AppletStub, AppletContext
{
    ...
    public Simulation_Frame(Applet applet, String[] args,
		     int width, int height) {
	build(applet, args, width, height, true);
    }

    /// Constructor with no default width/height.
    public Simulation_Frame(Applet applet, String[] args ) {
	build(applet, args, -1, -1,true);
    }

    /// Constructor with no arg parsing.
    public Simulation_Frame(Applet applet, int width, int height) {
	build( applet, null, width, height, true );
    }
    
    /// Constructeur additionnel, permet d'instancier une fenetre sans decoration.
    public Simulation_Frame(Applet applet, int width, int height, boolean decorated) {
	build( applet, null, width, height, decorated );
    }
	...
	*/
	...
	addWindowListener(new WindowAdapter() {
		...
		if (Simulation_Frame.this.applet != null) {
		    Simulation_Frame.this.applet.destroy();
		}
